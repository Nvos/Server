...
## REAL LOCAL DEPLOTMENT

Setup:

1. mkdir -p takdata/logs takdata/certs
2. docker compose up --build
3. generate client certificate:

```sh
cd /opt/tak/certs
sudo STATE=PL CITY=Bialystok ORGANIZATION=TAK ORGANIZATIONAL_UNIT=takserver \
     CAPASS=change_me_ca PASS=atakatak \
     ./makeCert.sh client omnitak
```

4. Add admin certificate to browser certificates `admin.p12`
5. Login into admin app `8443`
6. Create user: `user` / `User1-User1-User1`

**NOTE:** if `CoreConfig.xml` in takdata doesn't match one from example then overwrite it with one from example

### ATAK

1. Update LocalTAK to match changes (server address + change generated certificates)
2. Zip it
3. In ATAK import zip

### Android OmniTAK

1. Use quick access (thats it)
